...
abstract contract OwnerHelper {
    ...
    modifier isGenesisOwner() {
        require(
            msg.sender == _genesisOwner,
            "OwnerHelper: caller not contract creator"
        );
        _;
    }
    ...
    modifier onlyOwner() {
        require(
            msg.sender == _owners[0] ||
                msg.sender == _owners[1] ||
                msg.sender == _owners[2],
            "OwnerHelper: caller is not one of owners"
        );
        _;
    }
    ...
    modifier agreementOnly() {
        if (_numOwners == 2) {
            require(
                bytes(ownerAgreements[0].functionCode).length > 0 &&
                    bytes(ownerAgreements[1].functionCode).length > 0,
                "Agreement not set"
            );
            require(
                ownerAgreements[0].yesno == ownerAgreements[1].yesno,
                "Opinions Do Not Match"
            );
            require(
                keccak256(bytes(ownerAgreements[0].functionCode)) ==
                    keccak256(bytes(ownerAgreements[1].functionCode)),
                "Functions Do Not Match"
            );
            require(
                ownerAgreements[0].parameters.length ==
                    ownerAgreements[1].parameters.length,
                "Parameter size do not match"
            );

            for (uint256 i = 0; i < ownerAgreements[0].parameters.length; i++) {
                require(
                    ownerAgreements[0].parameters[i] ==
                        ownerAgreements[1].parameters[i],
            ...
            require(
                bytes(ownerAgreements[0].functionCode).length > 0 &&
                    bytes(ownerAgreements[1].functionCode).length > 0 &&
                    bytes(ownerAgreements[2].functionCode).length > 0,
                "Agreement not set"
            );

            require(
                ownerAgreements[0].yesno == ownerAgreements[1].yesno &&
                    ownerAgreements[0].yesno == ownerAgreements[2].yesno,
                "Opinions Do Not Match"
            );
            ...
            require(
                keccak256(bytes(ownerAgreements[0].functionCode)) ==
                    keccak256(bytes(ownerAgreements[1].functionCode)) &&
                    keccak256(bytes(ownerAgreements[0].functionCode)) ==
                    keccak256(bytes(ownerAgreements[2].functionCode)),
                "Functions Do Not Match"
            );
            ...
            require(
                ownerAgreements[0].parameters.length ==
                    ownerAgreements[1].parameters.length &&
                    ownerAgreements[0].parameters.length ==
                    ownerAgreements[2].parameters.length,
                "Parameter size do not match"
            );
            ...
            for (uint256 i = 0; i < ownerAgreements[0].parameters.length; i++) {
                ...
                require(
                    ownerAgreements[0].parameters[i] ==
                        ownerAgreements[2].parameters[i],
                    "Function Parameters Do Not Match"
                );
            }
    ...
    function addGenesisOwners(address newOwner)
        public
        isGenesisOwner
        returns (bool)
    {
    ...
    function getOwnerIndex(address Owner)
        private
        view
        onlyOwner
    ...
    function addAgreement(
        ...
        uint256[] memory decision,
        string memory comment
    ) public onlyOwner {
        uint256 index = getOwnerIndex(msg.sender);
        ownerAgreements[index] = AgreementData(
            yesno,
            functionCode,
            decision,
            comment
...
contract SimpleToken is ERC20Interface, OwnerHelper {
    ...
    function testOne() public view onlyOwner returns (string memory) {
        return "hi";
    }
        ...
        returns (string memory)
    {
        ...
        require(
            keccak256(bytes(ownerAgreements[0].functionCode)) ==
                keccak256(bytes(testTwoCode)),
            "Consensus reached, but wrong function call"
        );
        return "Test Two: multi-sig consensus success";
    }

    function testThree(uint256 param1, uint256 param2)
        public
        view
        onlyOwner
        agreementOnly
        returns (uint256)
    {
        ...
        require(
            param1 == ownerAgreements[0].parameters[0] &&
                param2 == ownerAgreements[0].parameters[1],
            "Consensus reached with correct function, but wrong parameter inputs"
        );
